...
- **Custom scripts**: `/config/.openclaw/scripts.d/*.sh` executed on startup (alphabetical order) and shutdown (reverse order). Scripts receive `start` or `stop` as $1. See `scripts.d.examples/` for examples.
